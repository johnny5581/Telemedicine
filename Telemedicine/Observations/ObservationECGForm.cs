...
namespace Telemedicine.Observations
{
    public partial class ObservationECGForm : FormBase
    {
        ...
        {
            var image = new PictureBox { Size = new Size(2600, 300) };
            var bmp = new Bitmap(2600, 300);
        ...
        private void DrawComponent(Bitmap b, Observation.ComponentComponent component)
        {
            using (var g = Graphics.FromImage(b))
            {
                ...
                if (sampleData != null)
                {
                    ...
                    // underline
                    var blockSize = h / 60; // 0.2 / 5 = 0.04s
                    var bPen = new Pen(Color.Red, 2);
                    var perX = 2.667f / 1000;
                    var ssw = perX / 0.04f;
                    // draw minor
                    for (var i = 0; i <= 50 * 5; i++)
                    {
                        var p = i % 5 == 0 ? bPen : Pens.Red;
                        var vx = i * blockSize;
                        g.DrawLine(p, vx, 0, vx, h);
                    }
                    ...
                    for (var i = 0; i < len; i++)
                    {
                        ...
                        var sec = i * perX;
                        var loc = sec / 0.2f;
                        var x = sx + loc * blockSize * 5;
                        var y = sy + ((value + 3000) * sh);
                        y = h - y;
